...
namespace Fallacy_Extractor
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            string str = pdfTool.Load(inputPdf);
            ...
            Root yaml = await n.parseToYAML(str) ?? throw new InvalidOperationException("Failed to deserialize YAML");
            foreach (string s in YAMLer.Validate(yaml))
            {
                Console.WriteLine(s);
            }
            List<Fallacy> fallacies = await n.FallacyDetect(yaml);
            foreach (Fallacy f in fallacies)
            {
                Console.WriteLine(f.ID + "\n" + f.Description + "\n at page: " + f.TextSpans[0].Page + "pertaining to: " + f.TargetNodes[0]);
            }
            yaml.Fallacies = fallacies;
            Pdf.test(inputPdf, outputPdf, pdfTool, yaml);


        }

    //     static async Task Main(string[] args) // test main
    //     {
    //         Console.Write("hello world");
    //         if (args.Length < 2)
    //         {
